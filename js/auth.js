const Auth = {
  ...
  async signInWithOAuth(provider) { if (!DB.isConnected() || !supabaseClient) throw new Error('OAuth requires Supabase connection'); const { data, error } = await supabaseClient.auth.signInWithOAuth({ provider, options: { redirectTo: window.location.origin + '/chat.html' } }); if (error) throw error; return data; },
