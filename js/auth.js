const Auth = {
  ...
  async init() {
    if (DB.isConnected() && supabaseClient) {
      ...
      supabaseClient.auth.onAuthStateChange((_event, session) => {
        this.currentUser = session?.user ? this._formatUser(session.user) : null;
  ...
  async signUp(email, password, name) {
    ...
    this.currentUser = { id: user.id, email: user.email, name: user.name, avatar: null };
  ...
  async signIn(email, password) {
    if (DB.isConnected() && supabaseClient) {
      ...
      if (error) throw error;
      this.currentUser = this._formatUser(data.user);
      this._notifyListeners();
      return data;
    }
    const users = Utils.storage.get('users') || [];
  ...
  async signInWithOAuth(provider) {
    if (!DB.isConnected() || !supabaseClient) {
      throw new Error('OAuth requires Supabase connection');
    }
    const { data, error } = await supabaseClient.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: 'https://myradiant.github.io/radiantwaves-ai/chat.html',
      },
    });
    if (error) throw error;
    return data;
